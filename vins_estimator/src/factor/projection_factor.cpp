...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    //第i帧IMU坐标系下的3D坐标
    Eigen::Vector3d pts_imu_i = qic * pts_camera_i + tic; //pi =qic* pc +tic
    //世界坐标系下的3D坐标
    Eigen::Vector3d pts_w = Qi * pts_imu_i + Pi; //Qi变换到世界坐标系的四元数，Pi世界坐标系的平移
    //第j帧imu坐标系下的3D坐标
    ...
    if (jacobians)
    {
        ...
        reduce << 1. / dep_j, 0, -pts_camera_j(0) / (dep_j * dep_j),
            0, 1. / dep_j, -pts_camera_j(1) / (dep_j * dep_j); //reduce 2*3矩阵
#endif
        ...
        if (jacobians[0])
        {
            ...
            jacobian_pose_i.rightCols<1>().setZero(); //对逆深度求导为0
        }
        ...
        if (jacobians[1])
        {
            ...
            jacobian_pose_j.rightCols<1>().setZero(); //对逆深度求导为0
        }
        ...
        if (jacobians[2])
        {
            ...
            jacobian_ex_pose.rightCols<1>().setZero(); //对逆深度求导为0
        }
