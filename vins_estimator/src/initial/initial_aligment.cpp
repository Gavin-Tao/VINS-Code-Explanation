...
void solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        Eigen::Quaterniond q_ij(frame_i->second.R.transpose() * frame_j->second.R); //second指ImageFrame
        //tmp_A = J_j_bw
        tmp_A = frame_j->second.pre_integration->jacobian.template block<3, 3>(O_R, O_BG);
        //tmp_b = 2 * ((r^bk_bk+1)^-1 * (q^c0_bk)^-1 * (q^c0_bk+1))_vec = 2 * ((r^bk_bk+1)^-1 * q_ij)_vec
...
void RefineGravity(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    for(int k = 0; k < 4; k++) //迭代4次
    {
        ...
        for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
        {
            ...
            b = b * 1000.0;
            x = A.ldlt().solve(b);
            VectorXd dg = x.segment<2>(n_state - 3);//从倒数第三个取两个，w1与w2两个正交基
...
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        tmp_b.block<3, 1>(0, 0) = frame_j->second.pre_integration->delta_p + frame_i->second.R.transpose() * frame_j->second.R * TIC[0] - TIC[0]; //TIC camera到imu的平移向量
        ...
        A.bottomRightCorner<4, 4>() += r_A.bottomRightCorner<4, 4>(); //右下角4x4
        b.tail<4>() += r_b.tail<4>(); //后4个元素

        A.block<6, 4>(i * 3, n_state - 4) += r_A.topRightCorner<6, 4>(); //右上角6x4
        A.block<4, 6>(n_state - 4, i * 3) += r_A.bottomLeftCorner<4, 6>(); //左下角4x6
    }
    A = A * 1000.0;
    b = b * 1000.0;
    x = A.ldlt().solve(b);
    double s = x(n_state - 1) / 100.0; //从倒数第一个取一个，前面除以100了，再乘上100
    ROS_DEBUG("estimated scale: %f", s);
    g = x.segment<3>(n_state - 4); //从倒数第四个开始取三个
    ROS_DEBUG_STREAM(" result g     " << g.norm() << " " << g.transpose()); //norm()范数，即模长
